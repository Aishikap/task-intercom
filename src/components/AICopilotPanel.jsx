...
const AICopilotPanel = () => {
  ...
  const [suggested, setSuggested] = useState("Can I cancel my order?");
  const [answer, setAnswer] = useState(null);
  const { setComposerText } = useChat();

  const sources = [
    "Canceling an order before it's shipped",
    "Modifying delivery details",
    "Requesting a cancellation refund"
  ];

  const generateAIAnswer = () => {
    setAnswer(`If you wish to cancel your order, you can do so before it is marked as shipped.

Please provide your order number to begin the cancellation process.

Note: Orders already dispatched cannot be canceled but may be eligible for return once delivered.`);
  };
    ...
    <div className="p-4 flex-1 overflow-y-auto space-y-6 text-sm text-gray-800 dark:text-gray-200">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-800 dark:text-gray-300">Emily Reyes</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1 text-gray-700 dark:text-gray-300">
          <div className="flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded px-2 -mx-2 py-1 transition">
            <FaEnvelope className="text-gray-500" /> alex.carter@example.com
          </div>
          <div className="flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded px-2 -mx-2 py-1 transition">
            <FaMapMarkerAlt className="text-gray-500" /> San Francisco
          </div>
          <div className="flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded px-2 -mx-2 py-1 transition">
            <FaMobileAlt className="text-gray-500" /> Android
          </div>
          <div className="flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded px-2 -mx-2 py-1 transition">
            <FaGlobe className="text-gray-500" /> Chrome
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1 text-gray-700 dark:text-gray-300">
          <div className="flex items-center gap-2">
            <FaBuilding className="text-gray-500" /> Nova Retail
          </div>
          <div className="flex items-center gap-2">
            <FaBriefcase className="text-gray-500" /> Online Marketplace
          </div>
          <div className="flex items-center gap-2">
            <FaTag className="text-gray-500" /> Plan: Enterprise
          </div>
        </div>
      </div>
    </div>
